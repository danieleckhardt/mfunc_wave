// Analogon zu testmfuncCHEB.cc fuer die Leja-Klasse.
//
// Gegenueber dem Cheb-Test gibt es GENAU VIER Unterschiede, alle in run():
//   (1) Kein estimate_degree_phi_k: Leja bestimmt den Grad adaptiv und
//       benoetigt keine Vorabschaetzung.
//   (2) Kein func-Parameter: matrix_func_leja ignoriert func (Kommentar
//       in expoInt.cpp: "func is not used in leja_approx, but we keep it
//       for consistency"), wird aber trotzdem uebergeben, damit die
//       Signatur identisch bleibt.
//   (3) matrix_func_leja statt matrix_func_cheb.
//   (4) Klassenname testLejaprotype statt testChebprotype.
//
// Alles andere (setup_system, create_ref_matrix_func, error_calc,
// matrix_vector_product, phi1, ...) ist Zeile fuer Zeile identisch,
// damit beide Tests dieselbe Referenzloesung und dieselbe Fehlermetrik
// verwenden.
...
#include "../../time_integration/expoInt_tools/leja_approx.h"  // [LEJA] chebyshev_approx.h -> leja_approx.h
...
// [LEJA] Klassenname: testLejaprotype statt testChebprotype
template <int dim>
class testLejaprotype
{
    public:
        testLejaprotype();
        double run();
    ...
    private:
        ...
        int    size;
        // [LEJA] kein int degree: Leja bestimmt den Grad adaptiv, keine
        //        Vorabschaetzung noetig (und kein estimate_degree_phi_k-Aufruf)
...
template<int dim>
testLejaprotype<dim>::testLejaprotype()
:
...
template<int dim>
int testLejaprotype<dim>::matrix_vector_product(Vector<double>& a, Vector<double>& b)
{
...
template<int dim>
int testLejaprotype<dim>::matrix_vector_product_complex_split(Vector<std::complex<double>>& a, Vector<std::complex<double>>& b)
{
    const unsigned int sz = a.size();

    Vector<double> a_real(sz), a_imag(sz);
    Vector<double> b_real(sz), b_imag(sz);
...
template<int dim>
int testLejaprotype<dim>::matrix_matrix_product(FullMatrix<std::complex<double>>& A)
{
    ...
    matrix_vector_product_complex_split(a, b);
    for (int i = 0; i < (int)A.m(); ++i) {
        A(i, 0) = a(i);
        A(i, 1) = b(i);
    }
...
template<int dim>
void testLejaprotype<dim>::setup_system()
{
    double coef_a       = 0.5;
...
template<int dim>
void testLejaprotype<dim>::create_ref_matrix_func(FullMatrix<double>& A, Vector<double>& b, Vector<double>& ref_vec_out)
{
    int two_size  = 2 * size;
    ...
    info = LAPACKE_dgeev(LAPACK_ROW_MAJOR, 'N', 'V', two_size,
                          A_temp.data(), lda,
                          wr.data(), wi.data(),
                          vl.data(), ldvl,
                          vr.data(), ldvr);
    ...
    while (col < two_size) {
        if (wi[col] == 0.0) {
            for (int j = 0; j < two_size; ++j)
                eigenvectors_ref(j, col) = {vr[j * two_size + col], 0.0};
            col++;
        } else {
            for (int j = 0; j < two_size; ++j) {
                eigenvectors_ref(j, col)     = { vr[j * two_size + col],  vr[j * two_size + col + 1] };
                eigenvectors_ref(j, col + 1) = { vr[j * two_size + col], -vr[j * two_size + col + 1] };
            }
            col += 2;
        }
    }
...
template<int dim>
double testLejaprotype<dim>::error_calc(Vector<double>& ref_vec_long, Vector<double>& calc_vec1, Vector<double>& calc_vec2)
{
    Vector<double> ref_vec1(size);
    Vector<double> ref_vec2(size);
    ...
    double err_total = std::sqrt(err_vec1.l2_norm() * err_vec1.l2_norm()
                                 + err_vec2.l2_norm() * err_vec2.l2_norm());
    double scale     = std::sqrt(std::pow(ref_vec1.l2_norm(), 2)
                                 + std::pow(ref_vec2.l2_norm(), 2));
    return err_total / scale;
}

template<int dim>
double testLejaprotype<dim>::run()
{
    ...
    expoint.find_opt_ellipse_prototype(ref_matrix_func);     // setzt ellipse_opt
    create_ref_matrix_func(ref_matrix_func, ref_vec_b, ref_vec_out);

    // [LEJA] kein estimate_degree_phi_k: der Grad wird residual-basiert in
    //        matrix_func_leja bestimmt; max_degree = 1000 ist bereits im
    //        Konstruktor von leja_approx gesetzt (expoInt.cpp, Zeile
    //        "leja_approx(1000, ...)").

    std::function<std::complex<double>(std::complex<double>)> func =
        [](std::complex<double> z) { return phi1(z); };   // [LEJA] uebergeben aber ignoriert

    auto vmult = [this](FullMatrix<std::complex<double>>& Ma) {
        return this->matrix_matrix_product(Ma);
    };

    // [LEJA] matrix_func_leja statt matrix_func_cheb
    expoint.matrix_func_leja(vmult, func, b_vec1, b_vec2, tol);
    std::cout.clear();
...
int main()
{
    ...
    // [LEJA] testLejaprotype statt testChebprotype
    testLejaprotype<dim> test;
    double err = test.run();
