...
template <typename VectorType>
void generateDiscreteCosin(VectorType &vec, double frequency = 1.0)
{
    const unsigned int N = vec.size();
    for (unsigned int i = 0; i < N; ++i)
    {
        double x = static_cast<double>(i ) / (N-1 ); // normalized grid point in (0, 1)
        vec[i] = std::cos(frequency * numbers::PI * x);
    }
}
...
std::complex<double> phi1(std::complex<double> x)
{
    ...
    if(std::abs(x)>1e-4)
        return  (exp(x)-1.)/x; // x/((x -1.)*(x -1.)*(x -1.));
...
class testRatKrylov
{
    public:
        testRatKrylov();
        std::vector<double> run(std::vector<int>& steps);
        double tol;
    private:
        int matrix_vector_product(Vector<double>& a, Vector<double>& b);
        int matrix_vector_product_shift(Vector<double>& a, Vector<double>& b);
        int matrix_vector_product_inv(Vector<double>& a, Vector<double>& b);
        void setup_system();
        ...
        FullMatrix<double>               ref_matrix_func_t;
        FullMatrix<double>               ref_matrix_func_tt;
        FullMatrix<double>               identity_m;
        std::unique_ptr<Types::InverseMatrix<dim,double>> M_inv;
        std::unique_ptr<Types::InverseMatrix<dim,double>> rat_krylov_inv;
        ...
        Vector<double>   ref_vec_out_t;
        Vector<double>   ref_vec_out_tt;
        // setup blockmatrix
...
template<int dim>
testRatKrylov<dim>::testRatKrylov()
:   
...
template<int dim> 
int testRatKrylov<dim>::matrix_vector_product(Vector<double>& a, Vector<double>& b ) {
...
template<int dim> 
int testRatKrylov<dim>::matrix_vector_product_shift(Vector<double>& a, Vector<double>& b ) {
    // factor *  [ [0 , I] [ -M^-1*homogeneous_matrix  , -M^-1* damping_matrix]  ] * [a , b]

    const Vector<double> old_a(a);
    const Vector<double> old_b(b);
...
template<int dim> 
int testRatKrylov<dim>::matrix_vector_product_inv(Vector<double>& a, Vector<double>& b ) {
    // factor *  [ [0 , I] [ -M^-1*homogeneous_matrix  , -M^-1* damping_matrix]  ] * [a , b]
    const Vector<double> old_a(a);
    const Vector<double> old_b(b);
    // b_1 = rat_krylov_inv * ( M * b_0 - time_step_size*rat_shift * A * a_0 )
    homogeneous_matrix.vmult(a,old_a);
    mass_matrix.vmult(b,old_b);
    a *= time_step_size*rat_shift;
    b -= a;
    rat_krylov_inv->vmult(b,b,1e-10);
    // a_1 = a_0 + time_step_size*rat_shift *b_1
    a = b;
    a *= time_step_size*rat_shift;
    a += old_a;
...
template<int dim>
void testRatKrylov<dim>::setup_system() {
    ...
    rat_krylov.copy_from(mass_matrix);
    rat_krylov.add(time_step_size * rat_shift, damping_matrix);
    rat_krylov.add(time_step_size * time_step_size * rat_shift * rat_shift, homogeneous_matrix);
    rat_krylov_inv = std::make_unique<Types::InverseMatrix<dim, double>>(rat_krylov,  dof_handler, false, true);
    ...
    ref_matrix_func_t.reinit(2*size,2*size);
...
template<int dim>   
void testRatKrylov<dim>::create_ref_matrix_func(FullMatrix<double>& A, Vector<double>& b, Vector<double>& ref_vec_out)
{
    // temp variable 
...
template<int dim>   
double testRatKrylov<dim>::error_calc(Vector<double> ref_vec_long, Vector<double> calc_vec1, Vector<double> calc_vec2)
{
    ...
    err_vec2 -= calc_vec2;
    Vector<double> temp_vec1 = err_vec1;  
...
template<int dim>
std::vector<double> testRatKrylov<dim>::run( std::vector<int>& steps) 
{
    ...
    auto vmult_pre =  [this](Vector<double>& a, Vector<double>& b) { return this->matrix_vector_product(a, b); };
    auto vmult_inv =  [this](Vector<double>& a, Vector<double>& b) {return this->matrix_vector_product_inv(a, b); };
    auto vmult =  [this](Vector<double>& a, Vector<double>& b) {return this->matrix_vector_product_shift(a,b); };
    // Debugging
    // Vector<double> test_vec1(size);
    // Vector<double> test_vec2(size);
    // generateRandomVectorself(test_vec1,0.,1.);
    // generateRandomVectorself(test_vec2,0.,1.);
    
    // Vector<double> test_vec(2*size);
    // for ( int i = 0; i < size; ++i){
    //     test_vec[i] = test_vec1[i];
    //     test_vec[i + size ] =  test_vec2[i];
    // }
    // vmult_inv(test_vec1,test_vec2);
    // vmult(test_vec1,test_vec2);
    // double error_test  = this->error_calc(test_vec,test_vec1,test_vec2);
    // std::cout << " Error of test = " << error_test ;
    //
    if ( steps.size() == 0){
        steps.push_back(2*size);
    }
    std::vector<double> err_steps;
    Vector<double> b_vec1_old(b_vec1);
    Vector<double> b_vec2_old(b_vec2);
    for( int& step : steps){
        expoint.set_max_iteration_krylov(step);
        expoint.matrix_func_rat_krylov(vmult,vmult_inv,func,b_vec1,b_vec2,tol,rat_shift);
        err_steps.push_back(this->error_calc(ref_vec_out,b_vec1,b_vec2));
        b_vec1 = b_vec1_old;
        b_vec2 = b_vec2_old;
    }
    std::cout.clear();
...
int main()
{
    ...
    std::vector<int> steps = {}; // for debugging, empty set means to choose 2*size as max_steps. (disable use_stopping_criteria in arnoldikrylov)
    
    testRatKrylov<dim> test;
    err = test.run(steps);
    if(err.back() <  test.tol )
        std::cout<< 1 ;
    else
        std::cout<< 0 << "  error = "<< err.back() ;
}
