...
double testexpoEuler<dim>::run()
{
    ...
    const double       refinement_steps      = 5.;
    ...
    {
        std::cout.setstate(std::ios_base::failbit);
        std::vector<int> config =  expoeuler_scheme.find_opt_ellipse_prototype();
        double degree = expoeuler_scheme.estimate_degree_phi_k(1,1e-2, 5);
        std::cout.clear();
