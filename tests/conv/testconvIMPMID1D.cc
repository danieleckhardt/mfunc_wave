...
:   
   ...
   ,fe(1)
   ,cn_scheme(fe, dof_handler,constraints,homogeneous_matrix,damping_matrix,mass_matrix,time_step_size,0,-1,false,false)
{}

template<int dim>
void testCN1<dim>::setup_system()
{
...
template<int dim>
void testCN1<dim>::cycle_time_steps(const double &time_step_size)
{
    time = 0.;
    cn_scheme.setup_system_macro_time(time_step_size);
    while(time < 1.0)       
    {
        time += time_step_size;
        std::cout.setstate(std::ios_base::failbit);
        cn_scheme.integrate_step(solution_u,solution_v,old_solution_u, old_solution_v, time);
        std::cout.clear();
...
template<int dim>   
void testCN1<dim>::process_solution(Solution<dim>& exact_solution)
{
    ...
    //std::cout<< std::to_string(solution_u.l2_norm())+ "&&";
    error_list.push_back(L2_error);
}

template<int dim>
double testCN1<dim>::EoC()
{
...
double testCN1<dim>::run()
{
    ...
    cn_scheme.setup_system_macro_space();
