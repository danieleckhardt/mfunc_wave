...
#include "../../time_integration/IMEXE.h"
...
template<int dim>
testIMEXE1<dim>::testIMEXE1()
:   
    dof_handler(triangulation)
   ,fe(1)
   ,imexE_scheme(fe, dof_handler,constraints,homogeneous_matrix,damping_matrix,mass_matrix,time_step_size,0,-1,false,false)
{}

template<int dim>
void testIMEXE1<dim>::setup_system()
{
...
template<int dim>
void testIMEXE1<dim>::cycle_time_steps(const double &time_step_size)
{
    time = 0.;
    imexE_scheme.setup_system_macro_time(time_step_size);
    while(time < 1.0)       
    {
        time += time_step_size;
        std::cout.setstate(std::ios_base::failbit);
        imexE_scheme.integrate_step(solution_u,solution_v,old_solution_u, old_solution_v, time);
        std::cout.clear();
...
template<int dim>   
void testIMEXE1<dim>::process_solution(Solution<dim>& exact_solution)
{
...
template<int dim>
double testIMEXE1<dim>::run()
{
    const unsigned int initial_refinement    = 10;
    ...
    imexE_scheme.setup_system_macro_space();
