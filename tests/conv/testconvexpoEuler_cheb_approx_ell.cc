...
template<int dim>
testexpoEuler<dim>::testexpoEuler()
:   
    dof_handler(triangulation)
   ,fe(1)
   ,expoeuler_scheme(fe, dof_handler,constraints,homogeneous_matrix,damping_matrix,mass_matrix,"cheb",time_step_size,0,-1,false,false)
{}

template<int dim>
void testexpoEuler<dim>::setup_system()
{
...
template<int dim>
void testexpoEuler<dim>::cycle_time_steps(const double &time_step_size)
{
    time = 0.;
...
template<int dim>   
void testexpoEuler<dim>::process_solution(Solution<dim>& exact_solution)
{
...
template<int dim>
double testexpoEuler<dim>::run()
{
    const unsigned int initial_refinement    = 7;
    ...
    std::cout.setstate(std::ios_base::failbit);
    std::vector<std::complex<double>> points =  expoeuler_scheme.find_opt_ellipse(tol);
    std::cout.clear();
    
    while(count_refinement_step < refinement_steps+1)
    {
        std::cout.setstate(std::ios_base::failbit);
        double degree = expoeuler_scheme.estimate_degree_phi_k(1,tol,degree);
        std::cout.clear();
        ...
        expoeuler_scheme.ellipse_opt.create_opt_ellipse(points);
    }
    return eoc = this->EoC();
}
