...
template <int dim>
class testexpoEulerratkrylov
{
    public:
        testexpoEulerratkrylov();
        double run();
    private:
        void setup_system();
...
template<int dim>
testexpoEulerratkrylov<dim>::testexpoEulerratkrylov()
:   
    dof_handler(triangulation)
   ,fe(1)
   ,expoeuler_scheme(fe, dof_handler,constraints,homogeneous_matrix,damping_matrix,mass_matrix,"rat_krylov", time_step_size,0,-1,true,false)
{}

template<int dim>
void testexpoEulerratkrylov<dim>::setup_system()
{
    ...
    damping_matrix *= 0.01; // beta
}

template<int dim>
void testexpoEulerratkrylov<dim>::cycle_time_steps(const double &time_step_size)
{
    time = 0.;
...
template<int dim>   
void testexpoEulerratkrylov<dim>::process_solution(Solution<dim>& exact_solution)
{
...
template<int dim>
double testexpoEulerratkrylov<dim>::EoC()
{
...
template<int dim>
double testexpoEulerratkrylov<dim>::run()
{
...
testexpoEulerratkrylov<dim> test;
eoc = test.run();
