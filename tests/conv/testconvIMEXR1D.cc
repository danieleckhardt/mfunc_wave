...
#include "../../time_integration/IMEXR.h"
...
template<int dim>
testIMEXR1<dim>::testIMEXR1()
:   
    dof_handler(triangulation)
   ,fe(1)
   ,imex_scheme(fe, dof_handler,constraints,homogeneous_matrix,damping_matrix,mass_matrix,time_step_size,0,-1,false,false)
{}

template<int dim>
void testIMEXR1<dim>::setup_system()
{
...
template<int dim>
void testIMEXR1<dim>::cycle_time_steps(const double &time_step_size)
{
    time = 0.;
    imex_scheme.setup_system_macro_time(time_step_size);
    while(time < 1.0)       
    {
        time += time_step_size;
        std::cout.setstate(std::ios_base::failbit);
        imex_scheme.integrate_step(solution_u,solution_v,old_solution_u, old_solution_v, time);
        std::cout.clear();
...
template<int dim>   
void testIMEXR1<dim>::process_solution(Solution<dim>& exact_solution)
{
    ...
    error_list.push_back(L2_error);
}

template<int dim>
double testIMEXR1<dim>::EoC()
{
...
template<int dim>
double testIMEXR1<dim>::run()
{
    const unsigned int initial_refinement    = 9;
    ...
    imex_scheme.setup_system_macro_space();
