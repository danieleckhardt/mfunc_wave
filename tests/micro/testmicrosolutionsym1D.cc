...
int main()//int argc, char *argv[]) // ändern?
{
    ...
    GeneratingHomCoeff<dim>   genHomCoeff(epsilon,delta,false, -1, level_micro,2, dof_handler_macro, fe_macro, constraints_macro,computing_timer);
    genHomCoeff.setup_system_and_triang_micro();
    genHomCoeff.assemble_B_H_symmetric(homogeneous_matrix);
    double test_value = homogeneous_matrix.frobenius_norm();
    std::cout<< test_value << std::endl; // Test against Laplace_Matrix*0.5
    }
    return 0;
}